    ...
    <div>
        ...
        border>
            ...
            <el-table-column
                ...
                min-width="100">
                <template slot-scope="scope" >
                    <el-button  type="text" size="small" @click="topar(scope.row.caseid)" v-if="scope.row.state==150">查看报价及方案</el-button>
                    <el-button  type="text" size="small" @click="tores(scope.row.caseid)" v-if="scope.row.state!=150">救援详情</el-button>
                </template>
            </el-table-column>
        </el-table>
        </div>
  ...
  methods: {
    ...
    getData(st, et) {
      ...
      fetch(constants.domain+"rescue/case/list_case", {
          method: "POST",
          body: `token=${this.token}&typeId=${this.typeId}&status=130&status=140&status=150`+tl,
          mode: "cors",
          ...
          .then(function(data) {
            console.log("请求了offer no start data:"+data);
            if (data.obj) {
              data.obj.map(v => {
                  console.log(v.obj);
                  n = n + 1;
