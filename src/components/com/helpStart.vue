		...
		<div >
			<table class="padding-10" >
				<tr>
					...
					<td>
						<el-select v-model="value"  @change="search()" placeholder="默认当前国家">
						  <el-option
					...
					<td>
						<el-select v-model="assistId1" placeholder="救援公司名称">
						  <el-option
				...
				<tr>
					...
					<td></td>
					<td>
						<el-select v-model="assistId2" placeholder="救援公司名称">
						  <el-option
				...
				<tr>
					...
					<td></td>
					<td>
						<el-select v-model="assistId3" placeholder="救援公司名称">
						  <el-option
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.axios
        .post(
          "http://api.test.dajiuxing.com.cn/rescue/bidding/submit_author",
          qs.stringify({
            token: this.token,
            id: this.caseid,
  ...
  mounted: function() {
    ...
    fetch(
      ...
      {
        method: "POST",
        body: `token=${this.token}&parentId=${this.obj.incidentType}`,
        mode: "cors",
        headers: { "Content-Type": "application/x-www-form-urlencoded" }
      }
    )
